    ...
    if schema_state.needs_bootstrap then
      if schema_state.legacy_invalid_state then
        error(fmt("cannot start Kong 1.x with a legacy %s, upgrade to 0.14 " ..
                  "first, and run 'kong migrations up'", db.infos.db_desc), 2)
      end

      if not schema_state.legacy_is_014 then
        error("database needs bootstrapping; run 'kong migrations bootstrap'", 2)
      end
    end

    if schema_state.new_migrations then
      error("new migrations available; run 'kong migrations up' to proceed", 2)
    end
  end
end
  ...
  if schema_state.needs_bootstrap then
    if schema_state.legacy_is_014 then
      error(fmt("cannot bootstrap a non-empty %s, run 'kong migrations " ..
                "up' instead", db.infos.db_desc), 2)
    end

    if schema_state.legacy_invalid_state then
      error(fmt("cannot bootstrap a non-empty %s, upgrade to 0.14 first, " ..
                "and run 'kong migrations up'", db.infos.db_desc), 2)
    end
...
local function up(schema_state, db, opts)
  if schema_state.needs_bootstrap then
    if schema_state.legacy_invalid_state then
      ...
      if schema_state.legacy_missing_migration then
        error(fmt("Migration to 1.0 can only be performed from a 0.14 %s " ..
                  ...
                  schema_state.legacy_missing_component, db.infos.db_desc), 2)
      end
      ...
      error(fmt("Migration to 1.0 can only be performed from a 0.14 %s " ..
                ...
                db.infos.db_desc, db.infos.db_desc), 2)
    end
    ...
    if schema_state.legacy_is_014 then
      ...
      error("cannot run migrations: database needs bootstrapping; " ..
            "run 'kong migrations bootstrap'", 2)
    end
  end
  ...
  local ok, err = db:cluster_mutex(MIGRATIONS_MUTEX_KEY, opts, function()
    ...
    if schema_state.pending_migrations then
      error("database has pending migrations; run 'kong migrations finish'", 2)
    end
...
local function finish(schema_state, db, opts)
  if schema_state.needs_bootstrap then
    if schema_state.legacy_invalid_state then
      -- legacy: migration from 0.14 to 1.0 cannot be performed
      log("cannot run migrations on a legacy %s", db.infos.db_desc)
      return
    end

    if schema_state.legacy_is_014 then
      log("cannot run migrations on a legacy %s; run 'kong migrations up' " ..
          "to proceed", db.infos.db_desc)
      return
    end

    log("cannot run migrations; run 'kong migrations bootstrap' instead")
    return
  end
